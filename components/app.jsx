...
let globalDraggedSettingId = null;
let globalDraggedReqId = null;

const formatRemainingTime = (totalDays) => {
    if (totalDays <= 0) return 'منتهي';
    let y = Math.floor(totalDays / 365);
    let m = Math.floor((totalDays % 365) / 30);
    let d = Math.floor((totalDays % 365) % 30);
    ...
    let parts = [yStr, mStr, dStr].filter(Boolean);
    return `متبقي ${parts.join(' و ')}`;
};

const calculateProgress = (startDateStr, endDateStr, durationDaysFallback) => {
    if (!startDateStr) return { remaining: 0, percent: 0, expired: true };
    const startDate = new Date(startDateStr);
    let endDate;
    if (endDateStr) {
        endDate = new Date(endDateStr);
    } else if (durationDaysFallback) {
        endDate = new Date(startDate.getTime() + (parseInt(durationDaysFallback) * 24 * 60 * 60 * 1000));
    } else {
        return { remaining: 0, percent: 0, expired: true };
    }
...
const getCombId = (platformId, planId, typeId, durationId) => {
    return `${platformId || 'null'}___${planId || 'null'}___${typeId || 'null'}___${durationId || 'null'}`;
};

const CustomSelect = ({ options, value, onChange, placeholder = "", required, disabled = false, customClass = "p-2.5" }) => {
            ...
            {isOpen && !disabled && (
                <div className="absolute left-0 right-0 w-full mt-1 bg-[#1e293b] border border-[var(--glass-border)] rounded-xl shadow-[0_15px_40px_rgba(0,0,0,0.6)] overflow-hidden animate-fadeIn top-full text-sm">
                    <div className="max-h-60 overflow-y-auto">
                        {options.length === 0 ? (
                            <div className="p-3 text-center text-[var(--text-muted)] text-sm font-bold">لا توجد خيارات</div>
                        ) : (
                            options.map((opt) => (
                                <div 
                                    key={opt.value}
                                    className={`p-3 transition-colors border-b border-[var(--glass-border)] last:border-0 flex items-center justify-between ${opt.disabled ? 'opacity-50 cursor-not-allowed bg-[rgba(0,0,0,0.2)]' : 'cursor-pointer hover:bg-[rgba(255,255,255,0.05)]'} ${String(value) === String(opt.value) && !opt.disabled ? 'bg-[rgba(56,189,248,0.15)] text-[var(--primary)] font-bold' : 'text-[var(--text-main)]'}`}
...
const CustomDatePicker = ({ value, onChange, disabled }) => {
    ...
    const handlePrevMonth = (e) => { e.stopPropagation(); setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1, 1)); };
    const handleNextMonth = (e) => { e.stopPropagation(); setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 1)); };
    const handleDateClick = (day, e) => {
        e.stopPropagation();
        const month = String(currentDate.getMonth() + 1).padStart(2, '0');
        const dayStr = String(day).padStart(2, '0');
        if (onChange) onChange(`${currentDate.getFullYear()}-${month}-${dayStr}`);
        setIsOpen(false);
    };
    return (
        <div className={`relative w-full h-full ${isOpen ? 'z-[9999]' : 'z-10'}`} ref={pickerRef}>
            <div 
                ...
                onClick={() => !disabled && setIsOpen(!isOpen)} dir="ltr"
            >
                <span className="text-sm font-bold">{value || 'YYYY-MM-DD'}</span>
                <i className="fas fa-calendar-alt text-[var(--text-muted)] text-sm"></i>
            </div>
            {isOpen && !disabled && (
                <div className="absolute left-0 right-0 mt-1 p-4 bg-[#1e293b] border border-[var(--glass-border)] rounded-xl shadow-[0_15px_40px_rgba(0,0,0,0.6)] animate-fadeIn z-[9999]" dir="ltr">
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center mb-2 border-b border-[var(--glass-border)] pb-2">
                        {dayNames.map(d => <div key={d} className="text-xs font-bold text-[var(--primary)]">{d}</div>)}
                    </div>
                    <div className="grid grid-cols-7 gap-1 text-center">
                        ...
                        {Array.from({ length: daysInMonth }).map((_, i) => {
                            ...
                            return (
                                <button key={day} type="button" onClick={(e) => handleDateClick(day, e)} className={`w-7 h-7 rounded-full text-xs flex items-center justify-center mx-auto transition-all duration-300 ${isSelected ? 'bg-[var(--primary)] text-white font-bold shadow-[0_0_10px_rgba(56,189,248,0.5)] scale-110' : isToday ? 'border border-[var(--primary)] text-[var(--primary)] font-bold' : 'text-[var(--text-main)] hover:bg-[rgba(255,255,255,0.1)] hover:scale-110'}`}>
                                    {day}
                                </button>
                            );
                        })}
...
const CustomNumberInput = ({ value, onChange, placeholder = "", min = 0 }) => {
    const handleIncrement = () => onChange(String((parseInt(value || 0) + 1)));
    const handleDecrement = () => onChange(String(Math.max(min, (parseInt(value || 0) - 1))));
    return (
...
const CombToggle = ({ id, label, checked, onChange, icon, disabled, onReorder }) => {
    return (
        <div 
            draggable={!disabled} onDragStart={(e) => { globalDraggedReqId = id; e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData('text/plain', id); }}
            onDragEnter={(e) => { e.preventDefault(); if (globalDraggedReqId && globalDraggedReqId !== id && !disabled) { onReorder(globalDraggedReqId, id); } }}
            onDragOver={(e) => e.preventDefault()} onDragEnd={() => { globalDraggedReqId = null; }}
            className={`flex items-center justify-between p-3 bg-[var(--inner-bg)] border rounded-2xl transition-all shadow-sm group relative ${disabled ? 'opacity-50 border-[var(--glass-border)]' : 'hover:border-[var(--primary)] hover:bg-[var(--input-bg)] border-[var(--glass-border)]'}`}
        >
            <label className={`flex-1 flex items-center justify-between min-w-0 pl-2 ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                ...
                <span className="font-bold text-[var(--text-main)] text-sm flex items-center gap-2 truncate flex-1 pr-1">
                    {icon && <i className={`${icon} text-[var(--text-muted)] ${!disabled ? 'group-hover:text-[var(--primary)]' : ''} transition-colors w-5 text-center text-sm`}></i>}
                    <span className="truncate">{label}</span>
                </span>
                <div className={`shrink-0 w-12 h-6 flex items-center rounded-full p-1 transition-colors duration-300 ${checked ? 'bg-[var(--primary)] shadow-[0_0_10px_rgba(56,189,248,0.3)]' : 'bg-[rgba(255,255,255,0.1)] border border-[var(--glass-border)]'}`}>
...
const SettingListItem = ({ item, path, onDelete, onEditClick, onReorder, isSelected, onClick, isBeingEdited }) => {
    ...
    return (
        <div 
            draggable={!isBeingEdited} onDragStart={(e) => { globalDraggedSettingId = item.id; e.dataTransfer.effectAllowed = "move"; e.dataTransfer.setData('text/plain', item.id); }}
            onDragEnter={(e) => { e.preventDefault(); if (globalDraggedSettingId && globalDraggedSettingId !== item.id) { onReorder(path, globalDraggedSettingId, item.id); } }}
            onDragOver={(e) => e.preventDefault()} onDragEnd={() => { globalDraggedSettingId = null; }}
            onClick={onClick} className={`flex items-center p-2.5 border rounded-xl cursor-pointer transition-all duration-300 mb-2 box-border px-3 ${selectedClasses}`}
        >
...
const MainContent = ({ user, auth, database }) => {
    ...
    React.useEffect(() => {
        if (selectedPlatformId && selectedPlanId && selectedTypeId && selectedDurationId) {
            const combId = getCombId(selectedPlatformId, selectedPlanId, selectedTypeId, selectedDurationId);
    ...
    const timeAgoFormatter = (dateStr) => {
        ...
        if (interval >= 1) return interval === 1 ? "منذ سنة" : interval === 2 ? "منذ سنتين" : "منذ " + interval + " سنوات";
        interval = Math.floor(seconds / 2592000);
        if (interval >= 1) return interval === 1 ? "منذ شهر" : interval === 2 ? "منذ شهرين" : "منذ " + interval + " أشهر";
        interval = Math.floor(seconds / 86400);
        if (interval >= 1) return interval === 1 ? "منذ يوم" : interval === 2 ? "منذ يومين" : "منذ " + interval + " أيام";
        interval = Math.floor(seconds / 3600);
        if (interval >= 1) return interval === 1 ? "منذ ساعة" : interval === 2 ? "منذ ساعتين" : "منذ " + interval + " ساعات";
        interval = Math.floor(seconds / 60);
        if (interval >= 1) return interval === 1 ? "منذ دقيقة" : interval === 2 ? "منذ دقيقتين" : "منذ " + interval + " دقائق";
        return "الآن";
    };
    ...
    React.useEffect(() => {
        if (activeTab === 'requests' && formData.startDate && (formData.durVal !== undefined)) {
            ...
            if (formData.durUnit === 'months') { d.setMonth(d.getMonth() + (parseInt(formData.durVal) || 0)); }
            else { d.setDate(d.getDate() + (parseInt(formData.durVal) || 0)); }
    ...
    const filterData = (dataArray) => {
        ...
        if (currentFilters.status !== 'all') {
            filtered = filtered.filter(item => {
                const prog = calculateProgress(item.startDate, item.endDate);
                if (currentFilters.status === 'active') return !prog.expired;
                if (currentFilters.status === 'expired') return prog.expired;
                return true;
            });
        }
        if (currentFilters.platform !== 'all') {
            filtered = filtered.filter(item => {
                const pId = item.platformId || settings.platforms.find(p => p.label === item.platform)?.id;
                return pId === currentFilters.platform;
            });
        }
        if (currentFilters.plan !== 'all') {
            if (activeTab === 'accounts') {
                filtered = filtered.filter(item => {
                    const pId = item.planId || settings.plans.find(p => p.label === item.plan && (p.platformId === item.platformId || p.platformName === item.platform))?.id;
                    return pId === currentFilters.plan;
                });
                ...
                filtered = filtered.filter(item => {
                    ...
                    if (!pId) {
                        if (item.plan && item.plan !== '-') {
                            pId = settings.plans.find(p => p.label === item.plan && (p.platformId === item.platformId || p.platformName === item.platform))?.id;
                        } else {
                            const reqAccount = accounts.find(a => a.id === item.accountId);
                            pId = reqAccount?.planId || settings.plans.find(p => p.label === reqAccount?.plan && (p.platformId === reqAccount?.platformId || p.platformName === reqAccount?.platform))?.id;
                        }
                    }
                    return pId === currentFilters.plan;
                });
        ...
        if (currentFilters.account !== 'all' && currentFilters.account !== undefined && activeTab === 'requests') {
            filtered = filtered.filter(item => item.accountId === currentFilters.account);
        }
        if (currentFilters.subType !== 'all' && currentFilters.subType !== undefined && activeTab === 'requests') {
            filtered = filtered.filter(item => {
                const sId = item.subscriptionTypeId || settings.subTypes.find(s => s.label === item.subscriptionType && (s.planId === item.planId || s.planName === item.plan))?.id;
                return sId === currentFilters.subType;
            });
        }
        if (currentFilters.search) {
            filtered = filtered.filter(item => Object.values(item).some(val => String(val).toLowerCase().includes(currentFilters.search.toLowerCase())));
        }
        return filtered;
    };
    ...
    const handleSave = (e) => {
        ...
        if (modalType === 'add') {
            ...
            newRef.set({ ...finalData, key: newRef.key });
        } else if (modalType === 'edit') {
            database.ref(`${path}/${finalData.id}`).update(finalData);
        }
    ...
    const handleDelete = (id) => {
        setConfirmDialog({ isOpen: true, message: 'تأكيد الحذف نهائياً', onConfirm: () => database.ref(`${activeTab}/${id}`).remove() });
    };

    const openModal = (type, data = {}) => {
        setModalType(type);
        setReqStep(activeTab === 'requests' && type === 'add' ? 'select' : 'fill_data');
        let initData = { ...data };
        if (type === 'add') {
            initData.startDate = new Date().toISOString().split('T')[0];
            initData.profileName = '';
            initData.profilePin = '';
            if (activeTab === 'requests') {
                initData.phoneNumber = initData.phoneNumber || "";
                initData.warrantyVal = initData.warrantyVal !== undefined ? initData.warrantyVal : ""; 
                initData.warrantyUnit = initData.warrantyUnit || "days";
                initData.amountPaid = initData.amountPaid !== undefined ? initData.amountPaid : (localStorage.getItem('lastAmountPaid') || "");
            }
    ...
    const handleDuplicate = (item) => {
        ...
        openModal('add', duplicatedItem);
        if (activeTab === 'requests') { setReqStep('fill_data'); }
    };
    ...
    const handleCancelEdit = () => {
        setEditTarget({path: null, id: null});
        setNewPlatformName(''); setNewPlatformUrl(''); setNewPlanName(''); setNewTypeName('');
    ...
    const handleEditClick = (item, path) => {
        ...
        if (path === 'platforms') { setNewPlatformName(item.label); setNewPlatformUrl(item.imageUrl || ''); }
        else if (path === 'plans') { setNewPlanName(item.label); }
        else if (path === 'subscriptionTypes') { setNewTypeName(item.label); }
        else if (path === 'durations') {
            setNewDurLabel(item.label); setNewDurVal(item.durVal || item.durMonths || item.durDays || '');
            setNewDurUnit(item.durUnit || (item.durMonths ? 'months' : 'days'));
            setNewWarVal(item.warrantyVal || ''); setNewWarUnit(item.warrantyUnit || 'days');
        }
    };
    ...
    const handleAddOrEditSetting = async (e, path, extraData = {}) => {
        e.preventDefault();
        let newValue = '';
        if (path === 'platforms') { if (!newPlatformName.trim()) return; newValue = newPlatformName.trim(); extraData.imageUrl = newPlatformUrl.trim(); }
        else if (path === 'plans') { if (!newPlanName.trim()) return; newValue = newPlanName.trim(); }
        else if (path === 'subscriptionTypes') { if(!newTypeName.trim()) return; newValue = newTypeName.trim(); }

        if (editTarget.id && editTarget.path === path) {
            let itemsList = [];
            if (path === 'platforms') itemsList = settings.platforms;
            if (path === 'plans') itemsList = settings.plans;
            if (path === 'subscriptionTypes') itemsList = settings.subTypes;
            const item = itemsList.find(i => i.id === editTarget.id);
            if (item) { await processEditSetting(path, item, newValue, extraData); }
            handleCancelEdit();
        } else {
            database.ref(`settings/${path}`).push({ label: newValue, value: newValue, order: Date.now(), ...extraData });
            if (path === 'platforms') { setNewPlatformName(''); setNewPlatformUrl(''); }
            if (path === 'plans') { setNewPlanName(''); }
            if (path === 'subscriptionTypes') { setNewTypeName(''); }
        }
    };

    const handleAddOrEditDuration = async (e) => {
        e.preventDefault();
        if (!newDurLabel.trim() || !selectedSettingType) return;
        ...
        if (editTarget.id && editTarget.path === 'durations') {
            const item = settings.durations.find(i => i.id === editTarget.id);
            if (item) { await processEditSetting('durations', item, newDurLabel.trim(), durData); }
            handleCancelEdit();
        } else {
            database.ref(`settings/durations`).push({
                label: newDurLabel.trim(), value: newDurLabel.trim(), ...durData,
                typeName: selectedSettingType, typeId: selectedTypeId, planName: selectedSettingPlan,
                planId: selectedPlanId, platformName: selectedSettingPlatform, platformId: selectedPlatformId, order: Date.now()
            });
            handleCancelEdit(); 
        }
    };
    ...
    const handleDeleteSetting = (path, id) => {
        setConfirmDialog({
            isOpen: true,
            message: 'تأكيد الحذف (سيتم حذف أي بيانات فرعية ورسائل التسليم المرتبطة بها نهائياً)',
            onConfirm: () => {
                const updates = {};
                updates[`settings/${path}/${id}`] = null;
                if (path === 'platforms') {
                    settings.plans.filter(p => p.platformId === id).forEach(p => updates[`settings/plans/${p.id}`] = null);
                    settings.subTypes.filter(s => s.platformId === id).forEach(s => updates[`settings/subscriptionTypes/${s.id}`] = null);
                    settings.durations.filter(d => d.platformId === id).forEach(d => updates[`settings/durations/${d.id}`] = null);
                    if(selectedPlatformId === id) { setSelectedPlatformId(null); setSelectedPlanId(null); setSelectedTypeId(null); setSelectedDurationId(null); }
                } else if (path === 'plans') {
                    settings.subTypes.filter(s => s.planId === id).forEach(s => updates[`settings/subscriptionTypes/${s.id}`] = null);
                    settings.durations.filter(d => d.planId === id).forEach(d => updates[`settings/durations/${d.id}`] = null);
                    if(selectedPlanId === id) { setSelectedPlanId(null); setSelectedTypeId(null); setSelectedDurationId(null); }
                } else if (path === 'subscriptionTypes') {
                    settings.durations.filter(d => d.typeId === id).forEach(d => updates[`settings/durations/${d.id}`] = null);
                    if(selectedTypeId === id) { setSelectedTypeId(null); setSelectedDurationId(null); }
                } else if (path === 'durations') {
                    if(selectedDurationId === id) setSelectedDurationId(null);
                }
                ...
                if (editTarget.id === id) handleCancelEdit();
            }
        });
    };
    ...
    const processEditSetting = async (path, item, newValue, extraData = {}) => {
        const updates = {};
        updates[`settings/${path}/${item.id}/label`] = newValue;
        updates[`settings/${path}/${item.id}/value`] = newValue;
        if (extraData && extraData.imageUrl !== undefined) updates[`settings/${path}/${item.id}/imageUrl`] = extraData.imageUrl;
        if (extraData && extraData.durVal !== undefined) updates[`settings/${path}/${item.id}/durVal`] = extraData.durVal;
        if (extraData && extraData.durUnit !== undefined) updates[`settings/${path}/${item.id}/durUnit`] = extraData.durUnit;
        if (extraData && extraData.warrantyVal !== undefined) updates[`settings/${path}/${item.id}/warrantyVal`] = extraData.warrantyVal;
        if (extraData && extraData.warrantyUnit !== undefined) updates[`settings/${path}/${item.id}/warrantyUnit`] = extraData.warrantyUnit;

        const oldLabel = item.label;
        if (path === 'platforms') {
            settings.plans.forEach(plan => { if (plan.platformId === item.id) updates[`settings/plans/${plan.id}/platformName`] = newValue; });
            settings.subTypes.forEach(sub => { if (sub.platformId === item.id) updates[`settings/subscriptionTypes/${sub.id}/platformName`] = newValue; });
            settings.durations.forEach(dur => { if (dur.platformId === item.id) updates[`settings/durations/${dur.id}/platformName`] = newValue; });
            accounts.forEach(acc => { if (acc.platform === oldLabel) updates[`accounts/${acc.id}/platform`] = newValue; });
            requests.forEach(req => { if (req.platform === oldLabel) updates[`requests/${req.id}/platform`] = newValue; });
        } else if (path === 'plans') {
            settings.subTypes.forEach(sub => { if (sub.planId === item.id) updates[`settings/subscriptionTypes/${sub.id}/planName`] = newValue; });
            settings.durations.forEach(dur => { if (dur.planId === item.id) updates[`settings/durations/${dur.id}/planName`] = newValue; });
            accounts.forEach(acc => { if (acc.plan === oldLabel && acc.platform === selectedSettingPlatform) updates[`accounts/${acc.id}/plan`] = newValue; });
            requests.forEach(req => { if (req.plan === oldLabel && req.platform === selectedSettingPlatform) updates[`requests/${req.id}/plan`] = newValue; });
        } else if (path === 'subscriptionTypes') {
            settings.durations.forEach(dur => { if (dur.typeId === item.id) updates[`settings/durations/${dur.id}/typeName`] = newValue; });
            requests.forEach(req => { if (req.subscriptionType === oldLabel && req.plan === selectedSettingPlan && req.platform === selectedSettingPlatform) updates[`requests/${req.id}/subscriptionType`] = newValue; });
        } else if (path === 'durations') {
            requests.forEach(req => {
                if (req.durationLabel === oldLabel && req.subscriptionType === selectedSettingType && req.plan === selectedSettingPlan && req.platform === selectedSettingPlatform) { updates[`requests/${req.id}/durationLabel`] = newValue; }
            });
        }
    ...
    const handleReorderSetting = (path, draggedId, targetId) => {
        ...
        let currentList = [];
        if (path === 'platforms') currentList = [...settings.platforms];
        else if (path === 'plans') currentList = settings.plans.filter(p => p.platformId ? p.platformId === selectedPlatformId : p.platformName === selectedSettingPlatform);
        else if (path === 'subscriptionTypes') currentList = settings.subTypes.filter(s => s.planId ? s.planId === selectedPlanId : (s.planName === selectedSettingPlan && s.platformName === selectedSettingPlatform));
        else if (path === 'durations') currentList = settings.durations.filter(d => d.typeId ? d.typeId === selectedTypeId : (d.typeName === selectedSettingType && d.planName === selectedSettingPlan && d.platformName === selectedSettingPlatform));

        const draggedIndex = currentList.findIndex(x => x.id === draggedId);
        const targetIndex = currentList.findIndex(x => x.id === targetId);
        if (draggedIndex === -1 || targetIndex === -1) return;
        const newList = [...currentList];
        const [removed] = newList.splice(draggedIndex, 1);
        newList.splice(targetIndex, 0, removed);
        const updates = {};
        newList.forEach((item, index) => { updates[`settings/${path}/${item.id}/order`] = index; });
                    ...
                    if (planTypes.length === 0) { combinations.push({ platform: platform.label, platformId: platform.id, plan: plan.label, planId: plan.id, type: '—', duration: '—', pImg: platform.imageUrl }); } else {
                        planTypes.forEach(type => {
                            ...
                            if (typeDurations.length === 0) { combinations.push({ platform: platform.label, platformId: platform.id, plan: plan.label, planId: plan.id, type: type.label, typeId: type.id, duration: '—', pImg: platform.imageUrl }); } else {
                                typeDurations.forEach(dur => {
                                    combinations.push({ 
                                        platform: platform.label, platformId: platform.id, plan: plan.label, planId: plan.id,
                                        type: type.label, typeId: type.id, duration: dur.label, durationId: dur.id,
                                        durVal: dur.durVal || dur.durMonths || dur.durDays || 0, durUnit: dur.durUnit || (dur.durMonths ? 'months' : 'days'),
                                        warrantyVal: dur.warrantyVal || 0, warrantyUnit: dur.warrantyUnit || 'days', pImg: platform.imageUrl 
                                    });
                                });
                            }
                        });
    ...
    const selectCombination = (comb) => {
        setFormData(prev => ({
            ...prev, platform: comb.platform, platformId: comb.platformId, plan: comb.plan === '—' ? '' : comb.plan, planId: comb.planId,
            subscriptionType: comb.type === '—' ? '' : comb.type, subscriptionTypeId: comb.typeId, durationLabel: comb.duration === '—' ? '' : comb.duration,
            durationId: comb.durationId, durVal: comb.durVal || 0, durUnit: comb.durUnit || 'days', warrantyVal: comb.warrantyVal || 0,
            warrantyUnit: comb.warrantyUnit || 'days', accountId: '', profilePosition: ''
        }));
    ...
    if (settings.requirementsOrder && settings.requirementsOrder.length > 0) {
        displayReqs.sort((a, b) => {
            let indexA = settings.requirementsOrder.indexOf(a.id);
            let indexB = settings.requirementsOrder.indexOf(b.id);
    ...
    const handleReorderRequirement = (draggedId, targetId) => {
        if (draggedId === targetId) return;
        const newOrderArray = [...displayReqs.map(r => r.id)];
        const draggedIndex = newOrderArray.indexOf(draggedId);
        const targetIndex = newOrderArray.indexOf(targetId);
        if (draggedIndex === -1 || targetIndex === -1) return;
        const [removed] = newOrderArray.splice(draggedIndex, 1);
        newOrderArray.splice(targetIndex, 0, removed);
        database.ref('settings/requirementsOrder').set(newOrderArray);
    };

    const updateLocalMessage = (textToSave) => {
        if (!selectedPlatformId || !selectedPlanId || !selectedTypeId || !selectedDurationId) return;
        const combId = getCombId(selectedPlatformId, selectedPlanId, selectedTypeId, selectedDurationId);
        database.ref(`settings/combinationFields/${combId}/deliveryMessage`).set(textToSave);
    };

    const insertVarToLocalMessage = (varText) => {
        const startPos = localDeliveryMsgRef.current.selectionStart;
        const endPos = localDeliveryMsgRef.current.selectionEnd;
        const newText = localDeliveryMsg.substring(0, startPos) + varText + localDeliveryMsg.substring(endPos, localDeliveryMsg.length);
        setLocalDeliveryMsg(newText); updateLocalMessage(newText);
        setTimeout(() => { localDeliveryMsgRef.current.focus(); localDeliveryMsgRef.current.selectionStart = startPos + varText.length; localDeliveryMsgRef.current.selectionEnd = startPos + varText.length; }, 0);
    };

    const generateAndShareSummary = async (item) => {
        let combId1 = getCombId(item.platformId, item.planId, item.subscriptionTypeId, item.durationId);
        let msg = settings.combinationFields[combId1]?.deliveryMessage;
        if (!msg) {
            const pObj = settings.platforms.find(p => p.label === item.platform);
            const plObj = settings.plans.find(p => (p.platformId === pObj?.id || p.platformName === item.platform) && p.label === item.plan);
            const tObj = settings.subTypes.find(s => (s.planId === plObj?.id || s.planName === item.plan) && s.label === item.subscriptionType);
            const dObj = settings.durations.find(d => (d.typeId === tObj?.id || d.typeName === item.subscriptionType) && d.label === item.durationLabel);
            const combId2 = getCombId(pObj?.id, plObj?.id, tObj?.id, dObj?.id);
            msg = settings.combinationFields[combId2]?.deliveryMessage;
        }
        msg = msg || '';
        const reqAccount = item.accountId && item.accountId !== '-' ? accounts.find(a => a.id === item.accountId) || {} : {};
        const startD = new Date(item.startDate);
        let warEndD = new Date(startD.getTime());
        if (item.warrantyUnit === 'months') { warEndD.setMonth(warEndD.getMonth() + parseInt(item.warrantyVal || 0)); }
        else { warEndD.setDate(warEndD.getDate() + parseInt(item.warrantyVal || 0)); }
        const warEndStr = warEndD.toISOString().split('T')[0];

        msg = msg.replace(/\[المنصة\]/g, item.platform || '');
        msg = msg.replace(/\[الخطة\]/g, item.plan && item.plan !== '-' ? item.plan : '');
        msg = msg.replace(/\[النوع\]/g, item.subscriptionType && item.subscriptionType !== '-' ? item.subscriptionType : '');
        msg = msg.replace(/\[المدة\]/g, item.durationLabel || '');
        msg = msg.replace(/\[اسم_المستخدم\]/g, reqAccount.email || '');
        msg = msg.replace(/\[كلمة_المرور\]/g, reqAccount.password || '');
        msg = msg.replace(/\[اسم_الملف\]/g, item.profileName && item.profileName !== '-' ? item.profileName : '');
        msg = msg.replace(/\[رمز_الملف\]/g, item.profilePin && item.profilePin !== '-' ? item.profilePin : '');
        msg = msg.replace(/\[تاريخ_البدء\]/g, item.startDate || '');
        msg = msg.replace(/\[تاريخ_الانتهاء\]/g, item.endDate || '');
        msg = msg.replace(/\[تاريخ_انتهاء_الضمان\]/g, warEndStr || '');
        ...
        if (isMobile) { window.location.href = waUrl; } else { const win = window.open(waUrl, '_blank'); if (!win) window.location.href = waUrl; }
    };

    let fPlatformId = settings.platforms.find(p => p.label === formData.platform)?.id;
    let fPlanId = settings.plans.find(p => p.label === formData.plan && (p.platformId === fPlatformId || p.platformName === formData.platform))?.id;
    let fTypeId = settings.subTypes.find(s => s.label === formData.subscriptionType && (s.planId === fPlanId || s.planName === formData.plan))?.id;
    let fDurId = settings.durations.find(d => d.label === formData.durationLabel && (d.typeId === fTypeId || d.typeName === formData.subscriptionType))?.id;
    const combIdForm = getCombId(fPlatformId, fPlanId, fTypeId, fDurId);
    const reqConfigForm = settings.combinationFields[combIdForm] || { reqAccount: true, reqProfilePos: true, reqPhone: true };
    const isCombSelected = selectedPlatformId && selectedPlanId && selectedTypeId && selectedDurationId;
    let currentReqConfig = { reqAccount: true, reqProfilePos: true, reqPhone: true };
    if (isCombSelected) {
        const currentCombId = getCombId(selectedPlatformId, selectedPlanId, selectedTypeId, selectedDurationId);
        currentReqConfig = settings.combinationFields[currentCombId] || currentReqConfig;
    }

    const moveProfile = (index, dir) => {
        const newProfiles = [...(formData.profiles || [])];
        if (index + dir >= 0 && index + dir < newProfiles.length) {
            const temp = newProfiles[index]; newProfiles[index] = newProfiles[index + dir]; newProfiles[index + dir] = temp;
        ...
        <div className="flex h-screen relative w-full max-w-[1600px] mx-auto z-10">
            <aside className={`admin-panel m-4 rounded-3xl flex flex-col z-20 shrink-0 shadow-2xl transition-all duration-300 w-24`}>
                ...
                <nav className="flex-1 flex flex-col p-4 gap-3 mt-2 overflow-y-auto">
                    ...
                    <button onClick={() => auth.signOut()} title="خروج" className="mt-auto w-full p-4 rounded-2xl transition-all duration-300 flex items-center justify-center text-sm font-bold text-[var(--error)] border border-transparent hover:border-[rgba(239,68,68,0.3)] hover:bg-[rgba(239,68,68,0.1)]">
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden py-4 pr-0 pl-4 z-10 transition-all duration-300">
                <div className="flex-1 overflow-y-auto relative rounded-3xl pb-4">
                        ...
                        <div className="flex flex-col h-full relative">
                            <div className="admin-panel p-4 rounded-3xl mb-6 flex flex-wrap justify-between items-center gap-4 shrink-0 z-20 shadow-lg">
                                ...
                                <div className="flex flex-wrap gap-3 flex-1 justify-end items-center">
                                    ...
                                    <div className="w-32 relative text-sm h-[42px] z-[100]">
                                        <CustomSelect customClass="px-3 h-full" options={[{ label: 'الكل', value: 'all' }, ...settings.platforms.map(p => ({ label: p.label, value: p.id }))]} value={currentFilters?.platform || 'all'} onChange={(val) => {setF('platform', val); setF('plan', 'all'); setF('subType', 'all'); setF('account', 'all');}} />
                                    </div>
                                    <div className="w-32 relative text-sm h-[42px] z-[99]">
                                        <CustomSelect customClass="px-3 h-full" disabled={currentFilters?.platform === 'all'} options={[{ label: 'الكل', value: 'all' }, ...settings.plans.filter(p => currentFilters?.platform === 'all' || p.platformId === currentFilters?.platform).map(p => ({ label: p.label, value: p.id }))]} value={currentFilters?.plan || 'all'} onChange={(val) => {setF('plan', val); setF('subType', 'all'); setF('account', 'all');}} />
                                    </div>
                                    {activeTab === 'requests' && (
                                    ...
                                    <div className="w-32 relative text-sm hidden sm:block h-[42px] z-[96]">
                                        <CustomSelect customClass="px-3 h-full" options={[{ label: 'الكل', value: 'all' }, { label: 'سارية', value: 'active' }, { label: 'منتهية', value: 'expired' }]} value={currentFilters?.status || 'all'} onChange={(val) => setF('status', val)} />
                                    </div>
                            ...
                            <div className="flex-1 overflow-y-auto pb-10">
                                {filterData(activeTab === 'accounts' ? accounts : requests).length === 0 ? (
                                    ...
                                    <Switch>
                                        <Route path="/accounts">
                                            <Accounts filterData={filterData} accounts={accounts} settings={settings} calculateProgress={calculateProgress} formatRemainingTime={formatRemainingTime} handleCopy={handleCopy} handleDuplicate={handleDuplicate} openModal={openModal} handleDelete={handleDelete} />
                                        </Route>
                                        <Route path="/requests">
                                            <Requests filterData={filterData} requests={requests} accounts={accounts} settings={settings} calculateProgress={calculateProgress} formatRemainingTime={formatRemainingTime} handleWhatsApp={handleWhatsApp} handleCopy={handleCopy} generateAndShareSummary={generateAndShareSummary} handleDuplicate={handleDuplicate} openModal={openModal} handleDelete={handleDelete} />
                                        </Route>
                                    </Switch>
                                )}
                            </div>
                    ...
                    {activeTab === 'settings' && (
                        <Settings 
                            settingsTab={settingsTab} setSettingsTab={setSettingsTab} settings={settings}
                            selectedPlatformId={selectedPlatformId} setSelectedPlatformId={setSelectedPlatformId} selectedPlanId={selectedPlanId} setSelectedPlanId={setSelectedPlanId} selectedTypeId={selectedTypeId} setSelectedTypeId={setSelectedTypeId} selectedDurationId={selectedDurationId} setSelectedDurationId={setSelectedDurationId}
                            editTarget={editTarget} handleAddOrEditSetting={handleAddOrEditSetting} newPlatformName={newPlatformName} setNewPlatformName={setNewPlatformName} newPlatformUrl={newPlatformUrl} setNewPlatformUrl={setNewPlatformUrl}
                            handleCancelEdit={handleCancelEdit} handleDeleteSetting={handleDeleteSetting} handleEditClick={handleEditClick} handleReorderSetting={handleReorderSetting}
                            selectedSettingPlatform={selectedSettingPlatform} newPlanName={newPlanName} setNewPlanName={setNewPlanName} selectedSettingPlan={selectedSettingPlan} newTypeName={newTypeName} setNewTypeName={setNewTypeName}
                            handleAddOrEditDuration={handleAddOrEditDuration} newDurLabel={newDurLabel} setNewDurLabel={setNewDurLabel} newDurVal={newDurVal} setNewDurVal={setNewDurVal} newDurUnit={newDurUnit} setNewDurUnit={setNewDurUnit}
                            newWarVal={newWarVal} setNewWarVal={setNewWarVal} newWarUnit={newWarUnit} setNewWarUnit={setNewWarUnit}
                            selectedSettingType={selectedSettingType} isCombSelected={isCombSelected} displayReqs={displayReqs} currentReqConfig={currentReqConfig}
                            handleToggleRequirement={handleToggleRequirement} handleReorderRequirement={handleReorderRequirement} localDeliveryMsg={localDeliveryMsg} setLocalDeliveryMsg={setLocalDeliveryMsg} updateLocalMessage={updateLocalMessage} insertVarToLocalMessage={insertVarToLocalMessage} localDeliveryMsgRef={localDeliveryMsgRef}
                            newCredEmail={newCredEmail} setNewCredEmail={setNewCredEmail} newCredPass={newCredPass} setNewCredPass={setNewCredPass} emailCreds={emailCreds} setConfirmDialog={setConfirmDialog} database={database}
                        />
                    )}

                    {activeTab === 'emails' && (
                        <Emails emailsData={emailsData} isFetchingEmails={isFetchingEmails} expandedEmailId={expandedEmailId} setExpandedEmailId={setExpandedEmailId} setConfirmDialog={setConfirmDialog} database={database} timeAgoFormatter={timeAgoFormatter} />
                    )}
                            ...
                            <form onSubmit={handleSave} className="p-6 overflow-y-auto flex-1 grid grid-cols-1 md:grid-cols-2 gap-5 bg-transparent">
                                {activeTab === 'accounts' ? (
                                    ...
                                    <>
                                                    ...
                                                    return (
                                                        <div key={req.id} className="col-span-1 sm:col-span-2 lg:col-span-1 relative z-[79]">
                                                            ...
                                                            {(() => {
                                                                ...
                                                                return (<CustomSelect options={accountOptions} value={formData.accountId} onChange={(val) => setFormData({...formData, accountId: val, profilePosition: '', profileName: '', profilePin: ''})} required={true} customClass="px-3 h-full text-sm font-bold" />);
                                                            })()}
                                                            </div>
                                                        </div>
                                                    );
                                        ...
                                        <div className="col-span-1 mt-1">
                                            ...
                                            <div className="h-[46px]"><CustomNumberInput value={formData.amountPaid} onChange={(val) => setFormData({...formData, amountPaid: val})} /></div>
                                        </div>
                                    </>
                                )}
                                ...
                                <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5 mt-1">
                                    <div className="col-span-1">
                                        ...
                                        <div className="h-[46px]"><CustomDatePicker value={formData.startDate || ''} onChange={(val) => setFormData({...formData, startDate: val})} /></div>
                                    </div>
                                    <div className="col-span-1">
                                        <label className="block text-sm font-bold text-[var(--text-main)] mb-2">الانتهاء</label>
                                        <div className="h-[46px]"><CustomDatePicker disabled={activeTab !== 'accounts'} value={formData.endDate || ''} onChange={(val) => {if(activeTab === 'accounts') setFormData({...formData, endDate: val})}} /></div>
                                    </div>
                                </div>
...
function MainApp({ user }) {
    return (
        <BrowserRouter>
            <MainContent user={user} auth={firebase.auth()} database={firebase.database()} />
        </BrowserRouter>
    );
}
...
function App() {
    ...
    if (!user) return <LoginScreen auth={firebase.auth()} />;
